var HT = (function () {
  ...
  var scienceData = [
      [0.0008686210640608003,18.235294117647058,0.0015472312703583065,18.235294117647058,'<div><a href="https://plot.ly/~JStevens/0/" target="_blank" title="&lt;br&gt;An Age Distribution for Scientific Genius" style="display: block; text-align: center;"><img src="https://plot.ly/~JStevens/0.png" alt="&lt;br&gt;An Age Distribution for Scientific Genius" style="max-width: 100%;width: 600px;"  width="600" onerror="this.onerror=null;this.src="\'https://plot.ly/404.png\'";" /></a><script data-plotly="JStevens:0" src="https://plot.ly/embed.js" async></script></div>'],
  ...
  var updateHeatmap = function(change, source) {
    if (change) {
  ...
  var point = function (min, max, value) {
    return (value - min) / (max - min);
  };

  var generateHeatmapData = function (colId) {
  ...
  me.demos = {
    hotSettings: {
      ...
      'science': {
        settings: {
            ...
            columnSorting: true,
            columns: [
              {data: 0, type: 'numeric', format: '0[.]000000000000000'},
              {data: 1, type: 'numeric', format: '0[.]000000000000000', renderer: heatmapRenderer},
              {data: 2, type: 'numeric', format: '0[.]000000000000000'},
              {data: 3, type: 'numeric', format: '0[.]000000000000000', renderer: heatmapRenderer},
              {data: 4, renderer: 'html'}
            ],
            afterLoadData: updateHeatmap,
            beforeChangeRender: updateHeatmap,
            mergeCells: [
              {row: 0, col: 4, rowspan: 20, colspan: 1}
            ]
        },
        getData: function () {
          return scienceData;
    ...
    index: function () {
      ...
      hotInstances['index'] = new Handsontable(container, {
        data: Handsontable.helper.createSpreadsheetData(12, 8),
        colHeaders: true,
        rowHeaders: true,
        height: 308,
        width: 500,
        afterLoadData: function () {
          setTimeout(function () {
            hotInstances['index'].selectCell(0, 0);
          }, 1000)
        }
      });
      ...
      Handsontable.Dom.addEvent(document.querySelector('#load-data'), 'click', function () {
        ...
        Handsontable.Dom.addClass(this, 'disabled');

        HT.scrollSmooth('header', function () {

          if (hotInstances['index']) {
            hotInstances['index'].destroy();
          }

          document.querySelector('.intro-container').style.top = '-2px';
    ...
    examples: function () {
      ...
      var htInstanceSettings = me.demos.hotSettings['basic'];
      ...
      tab: {
        ...
        callback: function (tab) {
          ...
          if (tab.hasClass('active') && !tab.attr('fired')) {
            ...
            var example = tab.attr('name').toLowerCase(),
                htInstanceSettings = me.demos.hotSettings[example];

            if (htInstanceSettings) {

              if (hotInstances['examples']) {
